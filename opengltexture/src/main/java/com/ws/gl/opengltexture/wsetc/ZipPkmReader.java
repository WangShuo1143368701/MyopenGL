 ...
 */
package com.ws.gl.opengltexture.wsetc;
...
public class ZipPkmReader {
    ...
    public void setZipPath(String path){
        Log.e("wangshuo",path+" set");
        this.path=path;
    }

    public boolean open(){
        Log.e("wangshuo",path+" open");
        ...
        try {
            if(path.startsWith("assets/")){
                ...
                Log.e("wangshuo",path+" is File exists->"+f.exists());
                mZipStream=new ZipInputStream(new FileInputStream(path));
            }
            return true;
        } catch (IOException e) {
            Log.e("wangshuo","eee-->"+e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
    ...
    private boolean hasElements(){
        try {
            if(mZipStream!=null){
                ...
                Log.e("wangshuo","mZip entry null");
            }
        } catch (IOException e) {
            Log.e("wangshuo","err  dd->"+e.getMessage());
            e.printStackTrace();
        }
        return false;
    }
    ...
    public ETC1Util.ETC1Texture getNextTexture(){
        if(hasElements()){
            try {
                ...
                Log.e("wangshuo","err->"+e1.getMessage());
                e1.printStackTrace();
            }
        }
        return null;
